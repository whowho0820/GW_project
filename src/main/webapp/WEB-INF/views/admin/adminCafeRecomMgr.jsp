			...
			<div style="text-align: center;">
				  ...
				  <!-- ex3 : cafeReview?page=${pageMaker.endPage+1 }&searchZone=${cri.searchZone }&searchTheme=${cri.searchTheme }&searchType=${cri.searchType }&keyword=${cri.keyword} -->
				  	<c:if test="${pageMaker.prev == true }">
						<li><a href="cafeRecomMgr?page=${pageMaker.startPage-1}&keyword=${cri.keyword}">&laquo;</a></li>
					</c:if>
					<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
						<li class="${pageMaker.cri.page == idx?'active':'' }"><a href="cafeRecomMgr?page=${idx}&keyword=${cri.keyword}">${idx }</a></li>
					</c:forEach>
					<c:if test="${pageMaker.next == true }">
						<li><a href="cafeRecomMgr?page=${pageMaker.endPage+1}&keyword=${cri.keyword}">&raquo;</a></li>
					</c:if>
			  	</ul>
			</div>
			<!-- 페이징 end -->
...
<script>
	...
	$(".closingBtn").click(function(){
		...
		if(res == true) {
			location.href = "${pageContext.request.contextPath}/admin/boardMgr/cafeRecomMgr/modify?cafeNo="+cafeNo+"&cafeCdt=2&page=${cri.page}&keyword=${cri.keyword}";
		} 
